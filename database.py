...
def db_query(query, params=(), fetch_one=False, commit=False):
    """یک تابع عمومی برای اجرای کوئری‌ها روی دیتابیس."""
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
        # این خط باعث می‌شود خروجی به صورت دیکشنری باشد تا دسترسی به ستون‌ها با نام ممکن شود
...
def init_db():
    """جداول بازیکنان و مأموریت‌ها را در صورت عدم وجود، ایجاد می‌کند."""
    # ساخت جدول بازیکنان با تمام آمار
    db_query('''
...
def get_player(user_id: int):
    """اطلاعات یک بازیکن را از دیتابیس می‌خواند و به صورت دیکشنری برمی‌گرداند."""
...
def update_player(user_id: int, data: dict):
    """اطلاعات یک بازیکن را در دیتابیس به‌روزرسانی می‌کند."""
    fields = ', '.join([f"{key} = ?" for key in data.keys()])
    values = list(data.values())
    values.append(user_id)
    db_query(f"UPDATE players SET {fields} WHERE user_id = ?", tuple(values), commit=True)

def create_player(user_id: int, name: str, location: str, x: int, y: int, max_hp: int, xp_to_next_level: int):
    """یک بازیکن جدید در دیتابیس ایجاد می‌کند."""
    db_query(
        """INSERT INTO players 
...
def get_active_quest(player_id: int, quest_name: str):
    """یک مأموریت فعال خاص را برای یک بازیکن برمی‌گرداند."""
    quest = db_query(
...
def add_quest(player_id: int, quest_name: str):
    """یک مأموریت جدید برای بازیکن اضافه می‌کند، به شرطی که از قبل فعال نباشد."""
...
def complete_quest(player_id: int, quest_name: str):
     """وضعیت یک مأموریت را برای بازیکن به 'completed' تغییر می‌دهد."""
     db_query(
